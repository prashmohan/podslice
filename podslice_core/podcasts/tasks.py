...
class AdManager:
    ...
    def _get_ad_segments_from_gemini(self, audio_path: str, audio_duration_seconds: float) -> str:
...
Example: [{{"start": 60.5, "end": 95.0}}]"""
    ...
    def _parse_ad_segments(self, gemini_response: str) -> List[Dict[str, float]]:
        logger.debug(f"Parsing Gemini response for Episode '{self.episode.title}' ({self.episode.id}): {gemini_response}")
        match = re.search(r'```json\n(\[.*?\])\n```|(\[.*?\])', gemini_response, re.DOTALL)
        if not match:
            logger.warning(f"No valid JSON array found in Gemini response for Episode '{self.episode.title}' ({self.episode.id}).")
            return []
        ...
        try:
            ...
            logger.warning(f"Gemini API returned invalid JSON for ad segments for Episode '{self.episode.title}' ({self.episode.id}). Response: {json_string}")
            return []

    def analyze_audio(self, audio_path: str, audio_duration_seconds: float) -> List[Dict[str, float]]:
        gemini_response = self._get_ad_segments_from_gemini(audio_path, audio_duration_seconds)
        self.episode.ad_segments = gemini_response
        self.episode.save(update_fields=['ad_segments'])
        logger.info(f"Gemini analysis complete for '{self.episode.title}'. Ad segments: {gemini_response}")
...
class EpisodeProcessor:
    ...
    def _update_status(self, status: Episode.Status, save: bool = True):
        ...
        logger.info(f"Episode '{self.episode.title}' ({self.episode.id}) status updated to {status.label}.")
    ...
    def _save_processed_audio(self, final_audio_path: str, file_size: int, output_mime_type: str = "audio/mpeg"):
        ...
        try:
            ...
            logger.info(f"Successfully created RehostedMedia record for '{self.episode.title}' (GUID: {media_entry.media_guid})")
        except Exception as e:
            logger.error(f"Failed to create RehostedMedia record for '{self.episode.title}': {e}", exc_info=True)
        ...
        self.episode.save(update_fields=['status', 'rehosted_audio_url', 'rehosted_audio_size', 'rehosted_media_id'])
        logger.info(f"Rehost completed successfully for Episode '{self.episode.title}' ({self.episode.id}).")

    def _download_audio(self) -> Optional[str]:
        self._update_status(Episode.Status.DOWNLOADING)
        try:
            logger.info(f"Downloading audio for '{self.episode.title}' from {self.episode.original_audio_url}")
            ...
            logger.info(f"Audio for Episode '{self.episode.title}' downloaded to {temp_file_path}.")
            return temp_file_path
        except requests.exceptions.RequestException as e:
            logger.error(f"Failed to download audio for Episode '{self.episode.title}': {e}", exc_info=True)
    ...
    def _get_audio_duration(self, audio_path: str) -> Optional[float]:
        try:
            ...
            if duration_str == 'N/A' or not duration_str:
                logger.error(f"ffprobe could not determine duration for '{self.episode.title}'. Output: '{duration_str}'")
                self._update_status(Episode.Status.FAILED)
                return None
                
            return float(duration_str)
        except subprocess.CalledProcessError as e:
            logger.error(f"ffprobe failed for '{self.episode.title}'. Stderr: {e.stderr}")
            self._update_status(Episode.Status.FAILED)
            return None
        except ValueError as e:
            logger.error(f"Could not convert ffprobe duration to float for '{self.episode.title}'. Output: '{result.stdout.strip()}'. Error: {e}")
            self._update_status(Episode.Status.FAILED)
            return None
        except Exception as e:
            logger.error(f"An unexpected error occurred while getting audio duration for '{self.episode.title}': {e}", exc_info=True)
            self._update_status(Episode.Status.FAILED)
            return None
    ...
    def _generate_ffmpeg_filter_complex(self, ad_segments: List[Dict[str, float]], audio_duration_seconds: float) -> str:
        content_segments = []
        last_segment_end = 0
    ...
    def _run_ffmpeg_slicing(self, audio_path: str, filter_complex: str) -> Tuple[str, int]:
        ...
        logger.info(f"Executing ffmpeg command for '{self.episode.title}': {' '.join(ffmpeg_cmd)}")
        try:
    ...
    def _save_original_audio_as_rehosted(self, audio_path: str):
        logger.info(f"No ad segments to slice for '{self.episode.title}'. Saving original audio.")
        final_audio_path = self._create_rehosted_media_path()
    ...
    def _save_empty_audio_as_rehosted(self):
        logger.warning(f"Ad segments covered the entire audio for '{self.episode.title}'. Result will be an empty file.")
        final_audio_path = self._create_rehosted_media_path()
        AudioSegment.empty().export(final_audio_path, format="mp3")
        file_size = os.path.getsize(final_audio_path)
        self._save_processed_audio(final_audio_path, file_size)

    def _slice_and_save_audio(self, audio_path: str, ad_segments: List[Dict[str, float]], audio_duration_seconds: float):
        if not ad_segments:
            self._save_original_audio_as_rehosted(audio_path)
            return

        logger.debug(f"Slicing audio for '{self.episode.title}' to remove {len(ad_segments)} ad segments.")
        filter_complex = self._generate_ffmpeg_filter_complex(ad_segments, audio_duration_seconds)
        
        if not filter_complex:
            logger.info("Did not get any ad segment slices for '{self.episode.title}'")
            self._save_empty_audio_as_rehosted()
        else:
            logger.info(f"Using the following ad segment slices for '{self.episode.title}': {filter_complex}")
            final_audio_path, file_size = self._run_ffmpeg_slicing(audio_path, filter_complex)
            self._save_processed_audio(final_audio_path, file_size)

    def rehost_audio(self):
        audio_path = None
        try:
            if self.episode.status in [Episode.Status.DOWNLOADING, Episode.Status.ANALYZING, Episode.Status.PROCESSING]:
                logger.warning(f"Skipping rehost task for '{self.episode.title}' because it is already in progress (status: {self.episode.get_status_display()}).")
                return
                
            logger.info(f"Starting rehost task for '{self.episode.title}' from podcast '{self.episode.podcast.title}'.")
            ...
            self._slice_and_save_audio(audio_path, ad_segments_list, audio_duration_seconds)

        except Exception as e:
            logger.error(f"An unexpected error occurred during rehosting of '{self.episode.title}': {e}", exc_info=True)
            self._update_status(Episode.Status.FAILED)
        finally:
            if audio_path and os.path.exists(audio_path):
                os.remove(audio_path)
            logger.info(f"Finished rehost task for Episode '{self.episode.title}' ({self.episode.id}).")
...
class FeedManager:
    ...
    def _get_episodes_to_process(self) -> set:
        episodes_to_process = set(
            self.podcast.episodes.filter(status__in=[Episode.Status.NEW, Episode.Status.FAILED])
        ...
        logger.info(f"Found {len(episodes_to_process)} existing episodes to process for '{self.podcast.title}'.")
    ...
    def _fetch_and_parse_feed(self) -> Optional[feedparser.FeedParserDict]:
        try:
            ...
            if feed.bozo:
                logger.error(f"Malformed feed for '{self.podcast.title}'. Reason: {feed.get('bozo_exception', 'Unknown')}")
                return None
            return feed
        except Exception as e:
            logger.error(f"Failed to fetch or parse feed for '{self.podcast.title}': {e}", exc_info=True)
            return None

    def _create_or_update_episode_from_entry(self, entry: feedparser.FeedParserDict) -> Optional[Tuple[Episode, bool]]:
        guid = entry.get("id")
        if not guid:
            logger.warning(f"Skipping entry in '{self.podcast.title}' due to missing GUID. Title: '{entry.get('title', 'N/A')}'")
            return None

        audio_url = next((enclosure.get("href") for enclosure in entry.get("enclosures", []) if enclosure.get("type", "").startswith("audio")), None)
        if not audio_url:
            logger.warning(f"Skipping entry '{entry.get('title', 'N/A')}' (GUID: {guid}) in '{self.podcast.title}' due to missing audio enclosure.")
            return None

        pub_date_parsed = entry.get("published_parsed")
        if not pub_date_parsed:
            logger.warning(f"Skipping entry '{entry.get('title', 'N/A')}' (GUID: {guid}) in '{self.podcast.title}' due to missing publication date.")
            return None
        ...
        try:
            ...
            logger.error(f"Failed to create episode with GUID {guid} for podcast '{self.podcast.title}'. Error: {e}", exc_info=True)
            return None

    def _process_feed_entries(self, feed: feedparser.FeedParserDict, episodes_to_process: set):
        new_episodes_count = 0
        skipped_episodes_count = 0
        logger.info(f"Processing the latest {5} episodes from '{self.podcast.title}'.")
        for entry in feed.entries[:5]:
            ...
            if result:
                ...
                if created:
                    ...
                    logger.info(f"New episode created: '{episode.title}' for podcast '{self.podcast.title}'.")
                    episodes_to_process.add(episode.id)
                else:
                    skipped_episodes_count += 1
        logger.info(f"Polling complete for '{self.podcast.title}'. Found {new_episodes_count} new episodes. Skipped {skipped_episodes_count} existing episodes.")
    ...
    def _enforce_episode_limit(self, limit: int = 5):
        all_episodes = self.podcast.episodes.all().order_by('-pub_date')
        if all_episodes.count() > limit:
            episodes_to_delete = all_episodes[limit:]
            logger.info(f"Enforcing episode limit. Deleting {len(episodes_to_delete)} old episodes for '{self.podcast.title}'.")
    ...
    def poll(self):
        ...
        if not lock.acquire(blocking=False):
            logger.warning(f"Polling for podcast {self.podcast.id} is already in progress. Skipping.")
            return
    ...
    def reprocess_podcast(self):
        logger.info(f"Starting reprocessing task for Podcast '{self.podcast.title}' ({self.podcast.id}).")
        ...
        logger.info(f"Completed dispatching reprocessing tasks for all episodes of podcast '{self.podcast.title}'.")
    ...
    def delete_podcast_data(self):
        ...
        self.podcast.delete()
        logger.info(f"Successfully deleted podcast '{podcast_title}' ({self.podcast.id}) and all its associated data.")
